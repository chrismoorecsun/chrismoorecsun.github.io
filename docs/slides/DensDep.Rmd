...
# Creating a density-dependent model, en Français
...
---
# The $r\textit{-}K$-logistic model
Another common representation of density-dependent change---i.e., a logistic model---is the $r\textit{-}K$ model. This model incorporates a "maximum" population size, $K$, below which the rate of change increases and above witch the population decreases. Here, it reads (the equation, the multiplied through):
$$\frac{\mathrm{d}N}{\mathrm{d}t} = rN\left(\frac{K - N}{K}\right) = rN - \frac{r}{K}N^2$$
--
We now see the equivalence to the $r\textit{-}\alpha$ model:
...
# Deriving the logistic model (from first principles)
Done in class, but to see it written with more detail, see Chris' .pdf notes under the miscellaneous column on the course schedule. [Here's a link to the .pdf.](https://chrismoorecsun.github.io/misc/Notes/Note2_LogisticUnits.pdf)
...
* Analyze it!
  * Simulate it
  * Solve it
  * Find equilibrium (long-term behavior)
...
.pull-left[
This is just stickin' it in `ode()`:
...
Let's find when the model doesn't change as the equilibrium, as a way to determine long-term behavior. If the rate of change is 0, then our non-stochastic models will stat at that value forever. All we gotta do is set the rate of change (derivative, on the LHS of the equation) to 0, then solve for our variable, $N$. Try it with the continuous logistic:

(As a hint, notice that the variable for which we are trying to solve, $N$ is squared, meaning, just like all those quadratic equations in algebra, there will be 2 solutions. Note you can reason this out without the quadratic/just followin' your gut.)
--
